...
#dateHandler() {
    ...
    if (this.#formData.days == undefined || this.#formData.days > maxDaysNew) {
        ...
        setOptionItems(this.#daysElement, range(0, maxDaysNew+1), "forecast days");
...
#hourFromHandler() {
    ...
    setOptionItems(this.#hourToElement, range(hour, 24), "hour to");
...
#hourToHandler() {
    ...
    if (this.#formData.hourFrom == undefined || this.#formData.hourFrom > hour) {
        ...
        setOptionItems(this.#hourFromElement, range(0, hour + 1), 'hour from')
    }
}
...
function setOptionItems (element, options, placeholder) {
    element.innerHTML = `<option value hidden detected>--${placeholder} --</option>`;
